...
Namespace Form
    Partial Public Class FrmReportMovieMarketShare
        Inherits Page

        Private ReadOnly _tableWidth As Unit = 920

#Region "Event Handlers"

        Protected Sub BtnCancelClick(ByVal sender As Object, ByVal e As EventArgs) Handles btnCancel.Click
            Response.Redirect("frmReportMovieMarketShareParam.aspx")
        End Sub
...
#Region "Methods"
        ...
        Private Function CreateEachGroup(ByVal displayName As String, ByVal pnl As Control, ByVal headerString As String, ByVal isAll As Boolean, ByVal dtb As DataTable) As GridView
            Dim grd As GridView = CreateGridView(False)
            If isAll Then
                grd.Columns.Add(CreateBoundField("Ranking", "RANKING", String.Empty, New Unit("10%"), HorizontalAlign.Center))
                grd.Columns.Add(CreateBoundField("DisplayName", displayName, String.Empty, New Unit("25%"), HorizontalAlign.Left))
                grd.Columns.Add(CreateBoundField("BoxOffice", "BOX OFFICE", "{0:#,##0}", New Unit("20%"), HorizontalAlign.Right))
                grd.Columns.Add(CreateBoundField("Adms", "ADMS", "{0:#,##0}", New Unit("15%"), HorizontalAlign.Right))
                'grd.Columns.Add(CreateBoundField("CountMovie", "NO. OF TITLES", "{0:#,##0}", New Unit("10%"), HorizontalAlign.Right))
                grd.Columns.Add(CreateBoundField("ATP", "ATP", "{0:#,##0.00}", New Unit("10%"), HorizontalAlign.Right))
                grd.Columns.Add(CreateBoundField("PercentKeyCityBo", "% KEY CITY (BO)", "{0:#,##0.00} %", New Unit("10%"), HorizontalAlign.Right))
                grd.Columns.Add(CreateBoundField("PercentKeyCityAdms", "% KEY CITY (ADMS)", "{0:#,##0.00} %", New Unit("10%"), HorizontalAlign.Right))
                grd.DataSource = dtb
                grd.DataBind()

                pnl.Controls.Add(CreateTableCaption(grd, headerString))
                pnl.Controls.Add(grd)
                pnl.Controls.Add(CreateTableFooter(grd, 2, 2, 3, 4))
            Else
                grd.Columns.Add(CreateBoundField("Ranking", "RANKING", String.Empty, New Unit("10%"), HorizontalAlign.Center))
                grd.Columns.Add(CreateBoundField("DisplayName", displayName, String.Empty, New Unit("15%"), HorizontalAlign.Left))
                grd.Columns.Add(CreateBoundField("BoxOffice", "BOX OFFICE", "{0:#,##0}", New Unit("15%"), HorizontalAlign.Right))
                grd.Columns.Add(CreateBoundField("Adms", "ADMS", "{0:#,##0}", New Unit("10%"), HorizontalAlign.Right))
                'grd.Columns.Add(CreateBoundField("CountMovie", "NO. OF TITLES", "{0:#,##0}", New Unit("10%"), HorizontalAlign.Right))
                grd.Columns.Add(CreateBoundField("ATP", "ATP", "{0:#,##0.00}", New Unit("10%"), HorizontalAlign.Right))
                grd.Columns.Add(CreateBoundField("PercentGroupBo", "% " + headerString + " (BO)", "{0:#,##0.00} %", New Unit("10%"), HorizontalAlign.Right))
                grd.Columns.Add(CreateBoundField("PercentGroupAdms", "% " + headerString + " (ADMS)", "{0:#,##0.00} %", New Unit("10%"), HorizontalAlign.Right))
                grd.Columns.Add(CreateBoundField("PercentKeyCityBo", "% KEY CITY (BO)", "{0:#,##0.00} %", New Unit("10%"), HorizontalAlign.Right))
                grd.Columns.Add(CreateBoundField("PercentKeyCityAdms", "% KEY CITY (ADMS)", "{0:#,##0.00} %", New Unit("10%"), HorizontalAlign.Right))
                ...
                pnl.Controls.Add(grd)
                pnl.Controls.Add(CreateTableFooter(grd, 2, 2, 3, 4))
                pnl.Controls.Add(CreateTableEmptyRow(grd))
            End If
            Return grd
        End Function

        Private Sub CreateResult(ByVal pnl As Control)
            Dim movieIdStr As String = Page.Request.QueryString("movieid").ToLower()
            Dim groupBy As String = Page.Request.QueryString("groupby").ToLower()
            ...
            Dim movieId As Int32? = Nothing
            Dim movieName As String = String.Empty
            Try
                movieId = CInt(movieIdStr)
                movieName = MovieAndTrailer.GetMovieDetail(movieId.Value).Rows(0)("movie_nameen").ToString()
            Catch ex As Exception
            End Try
            Select Case groupBy
            Case "circuit", "theater"
                reportTitle = String.Format("""{0}"" MARKET SHARE BY {1}", movieName, groupBy.ToUpper())
                ds = ReportGeneralIndustryInfo.ReportGeneralIndustryInfoByMovieCircuitTheater(movieId, dateFrom, dateTo, byMovieReleaseDate, groupBy = "circuit")

                Dim viewGroup As New DataView(ds.Tables(0))
                Dim dtbGroup As DataTable = viewGroup.ToTable(True, "GroupType")

                For Each dr As DataRow In dtbGroup.Rows
                    Dim groupType As String = dr("GroupType").ToString()
                    viewGroup.RowFilter = String.Format("GroupType = '{0}'", groupType)
                    grdData = CreateEachGroup(groupBy.ToUpper(), pnl, groupType.ToUpper(), False, viewGroup.ToTable())
                Next

                CreateEachGroup(groupBy.ToUpper(), pnl, "KEY CITIES", True, ds.Tables(1))
            Case "nationality"
