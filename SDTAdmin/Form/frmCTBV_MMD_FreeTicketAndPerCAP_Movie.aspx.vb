...
Namespace Form
    Partial Public Class FrmCtbvMmdFreeTicketAndPerCapMovie
        Inherits Page
        ...
        Protected Sub BtnUpdateClick(ByVal sender As Object, ByVal e As ImageClickEventArgs) Handles btnUpdate.Click
            For Each gridViewRow As GridViewRow In grdResult.Rows
                ...
                If chk IsNot Nothing AndAlso chk.Checked Then
                    ...
                    FreeTicketAndPerCapByMovie.UpdateInsert(CInt(strArr(0)), CInt(strArr(1)), CInt(strArr(2)), dpkUpdateAvailableTo.SelectedDate, CStr(Session("UserID")))
                End If
            Next
            ShowData()
        End Sub
...
#Region "Methods"
            ...
            If theaterId = -1 Then
                For Each ticketId As Int32 In ticketList
                    For i As Int32 = 1 To ddlTheater.Items.Count - 1
                        ...
                        FreeTicketAndPerCapByMovie.UpdateInsert(moviePopup.MovieId, ticketId, CInt(item.Value), dpkAddAvailableTo.SelectedDate, userId)
                    Next
                Next
            Else
                For Each ticketId As Int32 In ticketList
                    FreeTicketAndPerCapByMovie.UpdateInsert(moviePopup.MovieId, ticketId, theaterId, dpkAddAvailableTo.SelectedDate, userId)
                Next
            End If
        ...
        Private Sub ShowData()
            ...
            Dim dtb As DataTable = FreeTicketAndPerCapByMovie.SelectData(moviePopup.MovieId, typeId, theaterId, circuitId, Nothing)

            If typeId Is Nothing Then
                Dim viewType As New DataView(dtb)
                Dim dtbType As DataTable = viewType.ToTable(True, "ticket_cap_id", "ticket_cap_name")
                ddlType.DataSource = dtbType
                ddlType.DataTextField = "ticket_cap_name"
                ddlType.DataValueField = "ticket_cap_id"

                Dim firstRow As DataRow = dtbType.NewRow()
                firstRow("ticket_cap_name") = "All"
                firstRow("ticket_cap_id") = -1
