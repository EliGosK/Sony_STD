...
Partial Public Class frmRptCompMarketSare
    Inherits System.Web.UI.Page

    '--- Commented by Wittawat W. on 2012/12/12 : Add filmtype and sound
    'Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    '    Dim cDB As New cDatabase
    '    Dim DrData As IDataReader = cDB.getBoxOfficeSharingComp(Session("msRptMovieID"), Format(Session("msRptStrDate"), "yyyyMMdd"), Format(Session("msRptEndDate"), "yyyyMMdd"), "BKK")
    '    Dim DrTotal As IDataReader = cDB.getBoxOfficeSharingComp(Session("msRptMovieID"), Format(Session("msRptStrDate"), "yyyyMMdd"), Format(Session("msRptEndDate"), "yyyyMMdd"), "Total")
    '    Dim grandTotalBox, grandTotalAdms, grandSTamt, grandSTadms, grandTdamt, grandTdadms As Double
    ...
    '    Dim sumSoundtrack_amt, SumThai_dub_amt, SumBox, sum3D_amt, sum3D_adms, SumTd_amt, SumTd_Adms As Decimal
    ...
    '    For i = 1 To rowCount
    '        tbl.Rows(i + 5).Cells(11).Text = Format(CDbl(tbl.Rows(i + 5).Cells(8).Text) * 100 / SumBox, "#,##0.00") & "%"
    '        tbl.Rows(i + 5).Cells(13).Text = Format(CDbl(tbl.Rows(i + 5).Cells(9).Text) * 100 / SumAdms, "#,##0.00") & "%"
    '        tbl.Rows(i + 5).Cells(12).Text = Format(CDbl(tbl.Rows(i + 5).Cells(8).Text) * 100 / grandTotalBox, "#,##0.00") & "%"
    '        tbl.Rows(i + 5).Cells(12).Visible = False
    '        tbl.Rows(i + 5).Cells(14).Text = Format(CDbl(tbl.Rows(i + 5).Cells(9).Text) * 100 / grandTotalAdms, "#,##0.00") & "%"
    '        tbl.Rows(i + 5).Cells(14).Visible = False
    '        sumPerBox = sumPerBox + CDbl(tbl.Rows(i + 5).Cells(8).Text) * 100 / SumBox
    '        sumPerAdms = sumPerAdms + CDbl(tbl.Rows(i + 5).Cells(9).Text) * 100 / SumAdms
    '        sumPerBoxAll = sumPerBoxAll + CDbl(tbl.Rows(i + 5).Cells(8).Text) * 100 / grandTotalBox
    '        sumPerAdmsAll = sumPerAdmsAll + CDbl(tbl.Rows(i + 5).Cells(9).Text) * 100 / grandTotalAdms
    '        sum3D_amt = sum3D_amt + CDbl(tbl.Rows(i + 5).Cells(6).Text)
    '        sum3D_adms = sum3D_adms + CDbl(tbl.Rows(i + 5).Cells(7).Text)
    '    Next
    ...
    '    totalRow.Cells(6).Text = Format(sum3D_amt, "#,##0")
    ...
    '    totalRow.Cells(7).Text = Format(sum3D_adms, "#,##0")
    ...
    '    cmHeaderRow.Cells(1).Text = "THEATRE"
    ...
    '    'cmHeaderRowPer.BackColor = Color.FromName("#CADCEF")
    ...
    '    Dim DrDataCM As IDataReader = cDBCM.getBoxOfficeSharingComp(Session("msRptMovieID"), Format(Session("msRptStrDate"), "yyyyMMdd"), Format(Session("msRptEndDate"), "yyyyMMdd"), "CM")
    '    Dim rowcountCm, SumAdmsCm As Double
    '    Dim SumBoxCm As Decimal
    '    SumAdmsCm = 0
    '    SumBoxCm = 0

    '    Dim cmsessionCount, cmSumAdms, cmSumSoundtrack_adms, cmSumThai_dub_adms, cmrowCount As Double
    '    Dim cmsumSoundtrack_amt, cmSumThai_dub_amt, cmSumBox, cmSum3D_amt, cmSum3D_adms As Decimal
    ...
    '        tbl.Rows(iCm + rowCount + 9).Cells(11).Text = Format(CDbl(tbl.Rows(iCm + rowCount + 9).Cells(8).Text) * 100 / cmSumBox, "#,##0.00") & "%"
    '        tbl.Rows(iCm + rowCount + 9).Cells(13).Text = Format(CDbl(tbl.Rows(iCm + rowCount + 9).Cells(9).Text) * 100 / cmSumAdms, "#,##0.00") & "%"
    '        tbl.Rows(iCm + rowCount + 9).Cells(12).Text = Format(CDbl(tbl.Rows(iCm + rowCount + 9).Cells(8).Text) * 100 / grandTotalBox, "#,##0.00") & "%"
    '        tbl.Rows(iCm + rowCount + 9).Cells(14).Text = Format(CDbl(tbl.Rows(iCm + rowCount + 9).Cells(9).Text) * 100 / grandTotalAdms, "#,##0.00") & "%"
    '        sumPerBoxCm += CDbl(tbl.Rows(iCm + rowCount + 9).Cells(8).Text) * 100 / cmSumBox
    '        sumPerAdmsCm += CDbl(tbl.Rows(iCm + rowCount + 9).Cells(9).Text) * 100 / cmSumAdms
    '        sumPerBoxCmAll += CDbl(tbl.Rows(iCm + rowCount + 9).Cells(8).Text) * 100 / grandTotalBox
    '        sumPerAdmsCmAll += CDbl(tbl.Rows(iCm + rowCount + 9).Cells(9).Text) * 100 / grandTotalAdms
    '    Next
    ...
    '    cmtotalRow.Cells(6).Text = Format(cmSum3D_amt, "#,##0")
    ...
    '    cmtotalRow.Cells(7).Text = Format(cmSum3D_adms, "#,##0")
    ...
    '    DrData = cDB.getBoxOfficeSharingComp(Session("msRptMovieID"), Format(Session("msRptStrDate"), "yyyyMMdd"), Format(Session("msRptEndDate"), "yyyyMMdd"), "CITY")
    ...
    '        SumTd_amt = SumTd_amt + CDbl(tbl.Rows(rows).Cells(6).Text)
    '        SumTd_Adms = SumTd_Adms + CDbl(tbl.Rows(rows).Cells(7).Text)
    '    Next
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim dtDetailBKK As DataTable = cDatabase.GetDataTable("exec spGetRptCompMarketShare " + movieId _
                                                           + ", '" + startDate + "'" _
                                                           + ", '" + endDate + "'" _
                                                           + ", 'BKK'")
        Dim dtDetailCM As DataTable = cDatabase.GetDataTable("exec spGetRptCompMarketShare " + movieId _
                                                           + ", '" + startDate + "'" _
                                                           + ", '" + endDate + "'" _
                                                           + ", 'CM'")
        Dim dtDetailCITY As DataTable = cDatabase.GetDataTable("exec spGetRptCompMarketShare " + movieId _
                                                           + ", '" + startDate + "'" _
                                                           + ", '" + endDate + "'" _
                                                           + ", 'CITY'")
    ...
    Private Sub GenerateReportBKK(ByVal dtDetail As DataTable, ByVal dtPivot As DataTable, ByVal widthReport As Integer, ByVal colspanReport As Integer)
        ...
        For i As Integer = 0 To dtDetail.Rows.Count - 1
            ...
            trDetail.Cells(1).Text = String.Format("{0}", dtDetail.Rows(i)("theater_name"))
    ...
    Protected Sub btnExport0_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnExport0.Click
        ...
        Response.AddHeader("content-disposition", "attachment;filename=Comp_MarketShare.xls")
