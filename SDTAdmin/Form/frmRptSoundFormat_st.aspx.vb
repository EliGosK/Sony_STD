Imports Web.Data
Partial Public Class frmRptSoundFormat_st
    ...
    '    'Dim dtb.Rows(i) As IDataReader = cDB.GetRptSoundFormat(wkMvID, Sd, Ed, "Theatre_Sound")

    '    Dim dtb As DataTable
    '    dtb = cDatabase.GetDataTable("EXEC spGetRptSoundFormat " + wkMvID + ",'" + Sd + "','" + Ed + "','Theatre_Sound'")
    ...
    '        detailsRow.Cells(1).Text = "<a href=frmRptSoundFormatBoxoffice.aspx?theaterid=" + dtb.Rows(i)("theater_id").ToString() + "&typeid=soundtrack>" + Format(dtb.Rows(i)("soundtrack_amt"), "#,##0") + "</a>"
    ...
    '        detailsRow.Cells(3).Text = "<a href=frmRptSoundFormatBoxoffice.aspx?theaterid=" + dtb.Rows(i)("theater_id").ToString() + "&typeid=thaidub>" + Format(dtb.Rows(i)("thai_dub_amt"), "#,##0") + "</a>"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Dim dtDetail As DataTable = cDatabase.GetDataTable("exec spGetRptSoundFormat " _
            ...
            + ", '" + endDate + "'" _
            + ", 'Theatre_Sound'")
        Dim dtPivot As DataTable = cDatabase.GetDataTable("select distinct sound_header_group from tblFilmTypeSound where sound_header_group is not null")
        ...
        Me.tableReport.Rows(0).Cells(0).Text = Session("msRptMovieName") & " Box Office By Theatre & Sound"
        ...
        For j As Integer = dtPivot.Rows.Count - 1 To 0 Step -1
            ...
            tdHeader2.Text = dtPivot.Rows(j)("sound_header_group")
            ...
            For j As Integer = 0 To dtPivot.Rows.Count
                ...
                If j <> dtPivot.Rows.Count Then
                    ...
                    trDetail.Cells(y + 0).Text = "<a href=""frmRptSoundFormatBoxoffice.aspx?" _
                        ...
                        + "&headergroup=" + String.Format("{0}", dtPivot.Rows(j)("sound_header_group")) _
                        + "&typeid=Theatre_Sound" _
                        + """>" _
                        + String.Format("{0:#,##0}", dtDetail.Rows(i)(String.Format("{0}", dtPivot.Rows(j)("sound_header_group")) + " Amount")) _
                        + "</a>"

                    trDetail.Cells.Add(New TableCell)
                    trDetail.Cells(y + 1).HorizontalAlign = HorizontalAlign.Right
                    trDetail.Cells(y + 1).Text = String.Format("{0:#,##0}", dtDetail.Rows(i)(dtPivot.Rows(j)("sound_header_group") + " Adms"))

                    trDetail.Cells.Add(New TableCell)
                    trDetail.Cells(y + 2).HorizontalAlign = HorizontalAlign.Right
                    trDetail.Cells(y + 2).Text = String.Format("{0:#,##0}", dtDetail.Rows(i)(dtPivot.Rows(j)("sound_header_group") + " Screen"))
                Else
                    trDetail.Cells.Add(New TableCell)
                    trDetail.Cells(y + 0).HorizontalAlign = HorizontalAlign.Right
        ...
        For j As Integer = 0 To dtPivot.Rows.Count
            ...
            If dtDetail.Rows.Count > 0 Then
                If j <> dtPivot.Rows.Count Then
                    Me.tableReport.Rows(x).Cells(y + 0).Text = String.Format("{0:#,##0}", dtDetail.Compute("sum([" & dtPivot.Rows(j)("sound_header_group") & " Amount])", ""))
                    Me.tableReport.Rows(x).Cells(y + 1).Text = String.Format("{0:#,##0}", dtDetail.Compute("sum([" & dtPivot.Rows(j)("sound_header_group") & " Adms])", ""))
                    'Me.tableReport.Rows(x).Cells(y + 2).Text = String.Format("{0:#,##0}", dtb.Compute("max([" & dtPivot.Rows(j)("sound_header_group") & " Screen])", ""))
                Else
