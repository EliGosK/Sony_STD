...
Partial Public Class frmRptSoundFormatBoxoffice
    ...
    '        tbl.Rows(1).Cells(0).Text = " Film 35 mm."

    '        wkSQL = " SELECT tblMovie.movie_id,"
    '        wkSQL += vbNewLine + " datediff(day,tblMovie.movie_strdate, ISNULL(tblRevenue.revenue_date, 0))as diffDate, "
    '        wkSQL += vbNewLine + " ISNULL(SUM(tblRevenue.revenue_adms), 0) AS rev_adms, "
    '        wkSQL += vbNewLine + " ISNULL(SUM(tblRevenue.revenue_amount), 0) AS rev_amount, "
    '        wkSQL += vbNewLine + " ISNULL(tblRevenue.revenue_date, 0) AS revDate, "
    '        wkSQL += vbNewLine + " ISNULL(MIN(LTRIM(RTRIM(tblRevenue.movie_system))),'') as movie_system"
    '        wkSQL += vbNewLine + " ,tblRevenue.theater_id"
    '        wkSQL += vbNewLine + " FROM tblRevenue left JOIN"
    '        wkSQL += vbNewLine + " tblMovie ON tblMovie.movie_id = tblRevenue.movies_id"
    '        wkSQL += vbNewLine + " WHERE  (tblMovie.movie_id = " + wkMvID + ") and (tblRevenue.theater_id = " + strTheatreId + ")"
    '        wkSQL += vbNewLine + " and ((tblRevenue.revenue_date >= '" + Sd + "'"
    '        wkSQL += vbNewLine + " and tblRevenue.revenue_date <= '" + Ed + "'))"
    '        wkSQL += vbNewLine + " and (LTRIM(RTRIM(tblRevenue.movie_system)) = '35 มม.')"
    '        wkSQL += vbNewLine + " GROUP BY tblRevenue.theater_id, tblMovie.movie_id, "
    '        wkSQL += vbNewLine + " datediff(day,tblMovie.movie_strdate, ISNULL(tblRevenue.revenue_date, 0)), "
    '        wkSQL += vbNewLine + " ISNULL(tblRevenue.revenue_date, 0) "
    '        wkSQL += vbNewLine + " order by revDate"


    '    ElseIf strTypeId = "digital" Then
    '        tbl.Rows(0).Cells(0).Text = Session("msRptMovieName")
    '        tbl.Rows(2).Cells(0).Text = "From Date : " & Format(Session("msRptStrDate"), "ddd dd-MMM-yyyy") & " To " & Format(Session("msRptEndDate"), "ddd dd-MMM-yyyy")
    '        tbl.Rows(1).Cells(0).Text = " Digital"
    ...
    '        wkSQL += vbNewLine + " and ((tblRevenue.revenue_date >= '" + Sd + "'"
    '        wkSQL += vbNewLine + " and tblRevenue.revenue_date <= '" + Ed + "'))"
    '        wkSQL += vbNewLine + " and (LTRIM(RTRIM(tblRevenue.movie_system)) = 'ดิจิตอล')"
    '        wkSQL += vbNewLine + " GROUP BY tblRevenue.theater_id, tblMovie.movie_id, "
    '        wkSQL += vbNewLine + " datediff(day,tblMovie.movie_strdate, ISNULL(tblRevenue.revenue_date, 0)), "
    '        wkSQL += vbNewLine + " ISNULL(tblRevenue.revenue_date, 0) "
    '        wkSQL += vbNewLine + " order by revDate"

    '    ElseIf strTypeId = "dim" Then
    '        tbl.Rows(0).Cells(0).Text = Session("msRptMovieName")
    '        tbl.Rows(2).Cells(0).Text = "From Date : " & Format(Session("msRptStrDate"), "ddd dd-MMM-yyyy") & " To " & Format(Session("msRptEndDate"), "ddd dd-MMM-yyyy")
    ...
    '        wkSQL += vbNewLine + " and ((tblRevenue.revenue_date >= '" + Sd + "'"
    '        wkSQL += vbNewLine + " and tblRevenue.revenue_date <= '" + Ed + "'))"
    '        wkSQL += vbNewLine + " and (LTRIM(RTRIM(tblRevenue.movie_system)) = '3 มิติ')"
    '        wkSQL += vbNewLine + " GROUP BY tblRevenue.theater_id, tblMovie.movie_id, "
    '        wkSQL += vbNewLine + " datediff(day,tblMovie.movie_strdate, ISNULL(tblRevenue.revenue_date, 0)), "
    '        wkSQL += vbNewLine + " ISNULL(tblRevenue.revenue_date, 0) "
    '        wkSQL += vbNewLine + " order by revDate"

    '    ElseIf strTypeId = "seventydim" Then
    '        tbl.Rows(0).Cells(0).Text = Session("msRptMovieName")
    '        tbl.Rows(2).Cells(0).Text = "From Date : " & Format(Session("msRptStrDate"), "ddd dd-MMM-yyyy") & " To " & Format(Session("msRptEndDate"), "ddd dd-MMM-yyyy")
    '        tbl.Rows(1).Cells(0).Text = " Film 70 mm."
    ...
    '        wkSQL += vbNewLine + " and ((tblRevenue.revenue_date >= '" + Sd + "'"
    '        wkSQL += vbNewLine + " and tblRevenue.revenue_date <= '" + Ed + "'))"
    '        wkSQL += vbNewLine + " and ((LTRIM(RTRIM(tblRevenue.movie_system)) = '70 มม. 3 มิติ') or (LTRIM(RTRIM(tblRevenue.movie_system)) = '70 มม. 2 มิติ'))"
    '        wkSQL += vbNewLine + " GROUP BY tblRevenue.theater_id, tblMovie.movie_id, "
    '        wkSQL += vbNewLine + " datediff(day,tblMovie.movie_strdate, ISNULL(tblRevenue.revenue_date, 0)), "
    '        wkSQL += vbNewLine + " ISNULL(tblRevenue.revenue_date, 0) "
    '        wkSQL += vbNewLine + " order by revDate"
    ...
    '        wkSQL += vbNewLine + " ISNULL(SUM(tblRevenue.revenue_amount), 0) AS rev_amount, "
    '        wkSQL += vbNewLine + " ISNULL(tblRevenue.revenue_date, 0) AS revDate, "
    '        wkSQL += vbNewLine + " ISNULL(MIN(LTRIM(RTRIM(tblRevenue.sound_type))),'') as sound_type"
    '        wkSQL += vbNewLine + " ,tblRevenue.theater_id"
    '        wkSQL += vbNewLine + " FROM tblRevenue left JOIN"
    '        wkSQL += vbNewLine + " tblMovie ON tblMovie.movie_id = tblRevenue.movies_id"
    '        wkSQL += vbNewLine + " WHERE  (tblMovie.movie_id = " + wkMvID + ") and (tblRevenue.theater_id = " + strTheatreId + ")"
    '        wkSQL += vbNewLine + " and ((tblRevenue.revenue_date >= '" + Sd + "'"
    '        wkSQL += vbNewLine + " and tblRevenue.revenue_date <= '" + Ed + "'))"
    '        wkSQL += vbNewLine + " and (LTRIM(RTRIM(tblRevenue.sound_type)) = 'ต้นฉบับ') "
    '        wkSQL += vbNewLine + " GROUP BY tblRevenue.theater_id, tblMovie.movie_id, "
    '        wkSQL += vbNewLine + " datediff(day,tblMovie.movie_strdate, ISNULL(tblRevenue.revenue_date, 0)), "
    '        wkSQL += vbNewLine + " ISNULL(tblRevenue.revenue_date, 0) "
    '        wkSQL += vbNewLine + " order by revDate"
    ...
    '        wkSQL += vbNewLine + " and ((tblRevenue.revenue_date >= '" + Sd + "'"
    '        wkSQL += vbNewLine + " and tblRevenue.revenue_date <= '" + Ed + "'))"
    '        wkSQL += vbNewLine + " and (LTRIM(RTRIM(tblRevenue.sound_type)) = 'พากย์ไทย') "
    '        wkSQL += vbNewLine + " GROUP BY tblRevenue.theater_id, tblMovie.movie_id, "
    '        wkSQL += vbNewLine + " datediff(day,tblMovie.movie_strdate, ISNULL(tblRevenue.revenue_date, 0)), "
    '        wkSQL += vbNewLine + " ISNULL(tblRevenue.revenue_date, 0) "
    '        wkSQL += vbNewLine + " order by revDate"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Dim dtDetail As DataTable = cDatabase.GetDataTable("exec spGetRptSoundFormatBoxOffice " _
                + movieId _
                + ", " + strTheatreId _
                + ", '" + startDate + "'" _
                + ", '" + endDate + "'" _
            ...
            Me.tableReport.Rows(0).Cells(0).Text = Session("msRptMovieName")
            Me.tableReport.Rows(2).Cells(0).Text = "From Date : " & Format(Session("msRptStrDate"), "ddd dd-MMM-yyyy") & " To " & Format(Session("msRptEndDate"), "ddd dd-MMM-yyyy")
            Me.tableReport.Rows(1).Cells(0).Text = "Box Office By " + strTheatreName + " & " + strHeaderGroup
            ...
            If dtDetail.Rows.Count > 0 Then
                ...
                'Me.tableReport.Rows(Me.tableReport.Rows.Count - 1).Cells(3).Text = String.Format("{0:#,##0}", dtDetail.Compute("max([revenue_screen])", ""))
            Else
