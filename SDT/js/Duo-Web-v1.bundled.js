...
var Duo = {
    ...
    ready: function() {
        ...
        var d_div = D('#duo_div');
        var btn = D('#_ctl0_ContentPlaceHolder1_btnLogin1');
        var hdResponse = D('#_ctl0_ContentPlaceHolder1_hdResponse');
        ...
        iframe.attr('src', src);
        if (Duo._duo_sig != undefined) {
            d_div.attr('style', "position: absolute;top: 50%;left: 50%;margin-left: -310px;margin-top: -310px;z-index:9000;display: block");
        }

        D.receiveMessage(function(msg) {
            var sig_response = msg.data + ':' + Duo._app_sig;
            hdResponse.val(sig_response);
            btn.click();
        }, 'https://' + Duo._host);
    }
};
